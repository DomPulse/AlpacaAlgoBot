...
class TestStrategy(bt.Strategy):
	...
	def next(self):
		...
		if not self.position:
			...
			if (self.ema25[0] - self.ema100[0])/self.ema100[0] > 0.1:
			...
			if self.ema25[0] < self.ema100[0]:
				# SELL, SELL, SELL!!! (with all possible default parameters)
				#self.log('SELL CREATE, %.2f' % self.dataclose[0])

				# Keep track of the created order to avoid a 2nd order
				self.order = self.close()
		if self.num_days == 900:
			print(self.num_in_market/self.num_days)
...
if __name__ == '__main__':
	...
	for ticker in potential_stocks:
		...
		print(ticker, 'Final Portfolio Value: %.2f' % cerebro.broker.getvalue())
